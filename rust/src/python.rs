...
///
/// Builds the adjacency list once and traverses the sources in parallel, so
/// this is much cheaper than calling `shortest_paths_rust` per source. Row `i`
/// holds the distances from `sources[i]`.
#[pyfunction]
fn shortest_paths_multi_rust(
    ...
    let distances = shortest_paths_from_sources(n, &edge_list, &sources, directed)?;
    Ok(distances.into_pyarray(py).to_owned())
}
